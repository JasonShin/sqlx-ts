...
### 2. sqlx-ts cannot deal with extremely complex type inferences

There are some extremely complex type inferences that I am not going to implement (yet) - and they will result in a simpler `any` or other primary type. You are always given an option to type override it with `-- @result <name>: number` annotation.
