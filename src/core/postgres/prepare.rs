...
pub fn prepare<'a>(conn: &RefCell<&mut PostgresConn>, sql: &SQL, should_generate_types: &bool, handler: &Handler) -> Result<(bool, Option<TsQuery>)> {
    let mut failed = false;

    let span = sql.span.to_owned();
    let prepare_query = format!("PREPARE sqlx_stmt AS {}", sql.query);
    let result = conn.borrow_mut().query(prepare_query.as_str(), &[]);
    ...
    if let Err(e) = result {
        ...
        conn.borrow_mut().query("DEALLOCATE sqlx_stmt", &[]).unwrap();
    }
